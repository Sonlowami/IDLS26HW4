...
class ASRTrainer(BaseTrainer):
    ...
    def __init__(self, model, tokenizer, config, run_name, config_file, device=None):
        ...
        self.ce_criterion = nn.CrossEntropyLoss(label_smoothing=config['loss']['label_smoothing'], ignore_index=tokenizer.pad_id)
    ...
    def _train_epoch(self, dataloader):
        ...
        for i, batch in enumerate(dataloader):
            ...
            amp_context = torch.autocast(device_type=self.device, dtype=torch.float16) if self.device == "cuda" else nullcontext()
            with amp_context:
                ...
                if self.ctc_weight > 0:
                    ctc_target_lengths = transcript_lengths.detach().cpu().tolist()
                    ctc_targets = torch.cat([
                        targets_golden[b, :length].reshape(-1)
                        for b, length in enumerate(ctc_target_lengths)
                    ], dim=0)
    ...
    def _validate_epoch(self, dataloader):
        ...
        if references and hypotheses:
            val_metrics = self._calculate_asr_metrics(references, hypotheses)
        else:
            val_metrics = {'word_dist': 0.0, 'wer': 0.0, 'cer': 0.0}
        ...
        with torch.inference_mode():
            for i, batch in enumerate(dataloader):
                ...
                if recognition_config['beam_width'] > 1:
                    # Keep the top-ranked beam after beam search so decoding sees a single sequence per sample.
