...
class LMTrainer(BaseTrainer):
    ...
    def __init__(self, model, tokenizer, config, run_name, config_file, device=None):
        ...
        optimizer_config  = config['optimizer'][optimizer_name]
        scheduler_name = config['scheduler']['name']
        scheduler_config = config['scheduler'][scheduler_name]
        allowed_optims = {
            'sgd': optim.SGD,
            "adam": optim.Adam,
            "adamw": optim.AdamW
        ...
        optimizer = allowed_optims.get(optimizer_name)
        scheduler = allowed_schedulers.get(scheduler_name)
        assert optimizer is not None, f"{optimizer_name} is not a supported optimizer"
        assert scheduler is not None, f"{scheduler_name} is not a supported scheduler"
    ...
    def _train_epoch(self, dataloader) -> Tuple[Dict[str, float], Dict[str, torch.Tensor]]:
        ...
        for i, batch in enumerate(dataloader):
            ...
            with torch.autocast(device_type=self.device, dtype=torch.float16):
                ...
                raw_loss = self.criterion(raw_preds.permute(0, 2, 1), targets_golden)
    ...
    def _validate_epoch(self, dataloader):
        ...
        for i, batch in enumerate(dataloader):
            ...
            with torch.inference_mode():
                ...
                # What is the shape of raw_preds and targets_golden? 
                # Would you need to change the shape of the inputs to the criterion?
                loss = self.criterion(raw_preds.permute(0, 2, 1), targets_golden)
        ...
        with torch.inference_mode():
            if generation_config.get('top_k', 0) > 0 or generation_config.get('top_p', 0) > 0:
                ...
                seqs, scores = generator.generate_sample(prompts,
                                                         ...
                                                         generation_config.get('top_k', 0),
                                                         generation_config.get("top_p", 0.0))
